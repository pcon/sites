...
Irssi::settings_add_str('amarok', 'amarok_ssh_client', 'pcon@tinnytim.usersys.redhat.com');
Irssi::settings_add_str('amarok', 'amarok_dcop_user', 'pcon');
...
sub cmd_amarok ($$$) {
    ...
    } elsif ($arg[0] eq 'playpause') {
        cmd('playPause');
	print CLIENTCRAP $preprint.'Playing/Pausing song.';
    } elsif ($arg[0] eq 'play') {
        cmd('play');
